...
dados_armas_sp_consolidado_leg <- dados_armas_sp |>
  dplyr::mutate(
...
{tab_regra_1_leg <- dados_armas_sp_consolidado_leg |>
  dplyr::mutate(
    arma_tipo_final = arma_tipo_join
  ) |>
  dplyr::filter(
    sn_disponivel == "Sim",
...
armas_final <- dados_armas_sp_consolidado_leg |>
  dplyr::left_join(
...
dados_final <- armas_final |>
  dplyr::left_join(dados_ocorrencias) |>
  dplyr::select(
...
writexl::write_xlsx(armas_final, "inst/dados_sp/20240926_dados_armas.xlsx")
